...
*<< -- SV: In case we include R4, I think this project lends itself very well to talking about *domain*-general effects as opposed to *test*-general effects. If we're talking about test-general I guess we would model one overarching latent variable. However, With the WJ we can divide them up into several ability-domains. -- >>*
...
*<< -- SV: I don't see these hypotheses in the introduction, especially H3 which seems way more specific than the types of 'principled exploratory' steps that you describe there. Is this with a reason? Do we have enough existing evidence to predict H3 and if so, should that be something that features in the intro? -- >>*
...
*<< -- SV: Do you think it's necessary to control for age within these time points before aggregating? I assume the gap between the first and fifth timepoint is quite wide? -- >>*
...
MN has intimate knowledge of this dataset. MN has published two papers using the dataset. MN also has one manuscript in  press and one under review using the data. MN has not analyzed or used the subtest variables in the current preregistration.

GI has intimate knowledge of this dataset. GI is a co-principal investigator on the project and has published many papers using the data. Variables analyzed in GI publications relate mostly to the dependent variables in this project. Those relevant to the current preregistration include:
...
*<< -- SV: We might discuss whether these equivalence bounds are the best ones. Hypothetically, there could be a true effect of, say, 0.07, that is a real adversity effect but too small to be practically interesting. Setting equivalence bounds to .10 however would potentially classify this as a true zero effect (practically equivalent to zero). Thus, I think it could be that we actually want to set more narrow boundaries. -->>*


### Q21: Statistical Power

See our full power analysis [here](power-analysis/) and the code [here](../scripts/power-simulation.R).
